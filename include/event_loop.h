...
public:
    ...
    int set_time_callback(timeval t, function<void()> cb)
    {
        timeval now, next;
        gettimeofday(&now, NULL);
        next = time_add(now, t);
    ...
    int set_time_run_every(int t, function<void()> cb)
    {
        ...
        cb_queue.push({next, bind(&timer_fd::run_every, this, a)});
        return 0;
    }
    int set_time_run_every(timeval t, function<void()> cb)
    {
        timeval now, next;
        gettimeofday(&now, NULL);
        callback a(t, cb);
...
private:
    ...
    void run_every(callback cb)
    {
        ...
        lock_guard tmp2(time_lock);
        cb_queue.push({next, bind(&timer_fd::run_every, this, cb)});
    }
